...
airparif <- read.csv("airparif_centre_8_12_2016.csv", sep = ";", stringsAsFactors = TRUE)
...
ggplot(aes(heure, PM10, color = factor(year)), data = airparif) +
...
qplot(data = airparif, as.numeric(PM10), binwidth = 5)
...
p <- qplot(data = airparif, x = as.factor(year), y = PM10, geom = "boxplot")
p
...
p <- ggplot(aes(PM10), data = airparif)
p + geom_freqpoly()
